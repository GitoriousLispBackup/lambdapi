...
#endif
#ifdef RASPBERRY_PI
#define SEXP_SKIP_TASK SEXP_MAKE_IMMEDIATE(9) /* LambdaPi task priority skip */
#endif
...
#ifdef RASPBERRY_PI
#define sexp_taskp(x)       (sexp_check_tag(x, SEXP_TASK))
#endif
...
#ifdef RASPBERRY_PI
#define sexp_task_stack(x)             (sexp_field(x, task, SEXP_TASK, stack))
#define sexp_task_sp(x)                (sexp_field(x, task, SEXP_TASK, sp))
#define sexp_task_priority(x)          (sexp_field(x, task, SEXP_TASK, priority))
#define sexp_task_state(x)             (sexp_field(x, task, SEXP_TASK, state))
#define sexp_task_id(x)                (sexp_field(x, task, SEXP_TASK, id))
#endif
...
#define SEXP_TASK_SLEEP 0
#define SEXP_TASK_RUN   1

typedef void(*sexp_task_entry_t)(sexp env);

sexp sexp_make_task(sexp context, sexp_task_entry_t entry, void(*exit)(void), sexp env, sexp_uint_t priority, sexp_uint_t stack_size);
sexp sexp_start_task(sexp context, sexp task);
sexp sexp_sleep_task(sexp context, sexp task);

sexp sexp_list_destructive_remove(sexp list, sexp obj);
